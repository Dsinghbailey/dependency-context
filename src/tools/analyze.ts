  ...
  try {
    ...
    for (const dep of dependencies) {
      try {
        ...
        if (!repo) {
          errors.push(
            `Could not find GitHub repository for ${dep.name}@${dep.version}`
          );
          errorCount++;
          continue;
        }
    ...
    const result: AnalyzeResult = {
      ...
      message: `Processed ${
        dependencies.length
      } dependencies. Successfully indexed: ${successCount}. Errors: ${errorCount}${
        errors.length > 0 ? `. Error details: ${errors.join("; ")}` : ""
      }`,
    };
